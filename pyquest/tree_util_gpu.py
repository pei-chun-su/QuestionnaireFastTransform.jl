...
import numpy as np
import cupy as cp
from cupyx.scipy import sparse as cp_sps          # GPU Sparse library
...
def tree_sums(data_gpu, row_tree):
    """
    Computes all tree folder sums in a single GPU operation.
    data_gpu: (num_rows x num_samples) array already in VRAM.
    """
    num_rows = data_gpu.shape[0]
    num_nodes = row_tree.tree_size
    
    # 1. Build the Sparse Indicator Matrix (S)
    # S[i, j] = 1 if row j is in node i
    rows = []
    cols = []
    for node in row_tree.traverse():
        node_indices = node.elements  # Indices of rows in this node
        rows.extend([node.idx] * len(node_indices))
        cols.extend(node_indices)
    
    # Create the matrix as a GPU Sparse Matrix (CSR format)
    # This matrix S has shape (num_nodes, num_rows)
    ones = cp.ones(len(rows), dtype=cp.float32)
    S = cp_sps.csr_matrix((ones, (cp.array(rows), cp.array(cols))), 
                          shape=(num_nodes, num_rows))
    
    # 2. Compute sums for all nodes at once
    # (nodes x rows) @ (rows x samples) -> (nodes x samples)
    # This triggers one highly optimized CUDA kernel.
    sums = S @ data_gpu
    
    return sums

def tree_averages(data_gpu, row_tree):
    # Get the sums using the optimized GPU function
    sums = tree_sums(data_gpu, row_tree)
    
    # Pre-calculate node sizes on CPU and move to GPU
    node_sizes = cp.array([node.size for node in row_tree.traverse()], dtype=cp.float32)
    
    # Element-wise division on GPU (Broadcasting)
    # sums is (nodes x samples), node_sizes is (nodes,)
    return sums / node_sizes[:, cp.newaxis]
