...
def pyquest(data,params):
    ...
    for i in range(params.n_iters):
        ...
        if params.col_affinity_type == DUAL_EMD:
            ...
            col_aff,[],[] = dual_affinity.partition_dualgeometry(np.expand_dims(data, axis=2), dual_row_trees[-1], params.col_alpha, 1)
        elif params.col_affinity_type == DUAL_COSINE:
            print("cosine distance.")
            col_aff,[],[] = dual_affinity.partition_dualgeometry(np.expand_dims(data, axis=2), dual_row_trees[-1], params.col_alpha, 0)
