"""UMAP of the (l,m) channel affinity (99 harmonics, version B), colored by degree l, |m| (meridians),
...
Y = umap.UMAP(n_components=2, metric='precomputed', n_neighbors=15, min_dist=0.1, random_state=0).fit_transform(D)
print("UMAP done, %d points" % NC, flush=True)
...
fig.suptitle(r'UMAP of the $(\ell,m)$ channel affinity (99 harmonics, version B $N=2048$): n\_neighbors 15, distance $=-\log$(affinity)', fontsize=14, y=1.0)
fig.tight_layout(rect=[0, 0, 1, 0.95])
fig.savefig(os.path.join(HERE, 'umap_channels.png'), dpi=140, bbox_inches='tight')
fig.savefig(os.path.join(HERE, 'umap_channels.pdf'), bbox_inches='tight'); print("SAVED")
